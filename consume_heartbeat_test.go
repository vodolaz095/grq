...
func TestGenerateOfflineQueue(t *testing.T) {
	rq3, err := NewFromOptions(t.Context(), testHeartbeatQueue, redis.Options{
		Network: "tcp",
		Addr:    "127.0.0.1:6379",
	})
	if err != nil {
	...
	for i := 0; i < testSendLimit; i++ {
		err = rq3.Publish(t.Context(), fmt.Sprintf("task %v created on %s", i, time.Now().Format(time.Stamp)))
		if err != nil {
			t.Error(err)
		}
		t.Logf("Task %v published", i)
	}
	err = rq3.PublishFirst(t.Context(), "this task will be executed as first one")
	if err != nil {
...
//	rq4, err := NewFromConnectionString(t.Context(), testHeartbeatQueue, DefaultConnectionString)
//	if err != nil {
//		t.Errorf("%s : while connecting to redis", err)
//	}
//	first, found, err := rq4.GetTask(t.Context())
//	if err != nil {
...
//	hbwg.Add(1)
//	go func() {
//		feed, err := rq4.Consume(t.Context())
//		if err != nil {
//			t.Error(err)
//		}
//		t.Logf("Feed created...")
...
//	}()
//	hbwg.Wait()
//	payload, found, err := rq4.GetTask(t.Context())
//	if payload != "" {
...
//	}
//
//	err = rq4.PublishFirst(t.Context(), "it will be rejected")
//	if err != nil {
